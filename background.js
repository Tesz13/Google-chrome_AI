...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // MESSAGE TYPE 3: Capture what they are looking at
  if (request.type === 'CAPTURE_VISIBLE') {
    (async () => {
      try {
        // Find the window this tab is in
        const windowId = sender.tab?.windowId ?? (await chrome.windows.getCurrent()).id;
        // Take a picture of the visible part of the page
        const dataUrl = await chrome.tabs.captureVisibleTab(windowId, { format: 'png' });
        sendResponse({ ok: true, dataUrl });  // Send back the picture
      } catch (err) {
        console.warn('CAPTURE_VISIBLE failed', err);
        sendResponse({ ok: false, error: String(err) });
      }
    })();
    return true; // Keep the mailbox open while we take the photo
  }
